...
func setup_levels() -> void:
	for i in SceneManager.level_total():
		var nb: LevelButton = LEVEL_BUTTON.instantiate()
		nb.level_number = i
		level_select_container.add_child(nb)
		if SceneManager.is_level_available(i):
			nb.unlock()
		else:
			nb.lock()
		nb.on_button_hover.connect(on_button_hover)
		nb.on_button_pressed.connect(on_button_pressed)
